...
class DomalainArpa3 extends Module
{
    ...
    public function hookUpdateQuantity($params)
    {
        ...
        if ($params['product']['active'] == 1) {
            if (file_exists(dirname(__FILE__) . '/mails/' . $lang . '/productmonitor.txt') AND file_exists(dirname(__FILE__) . '/mails/' . $lang . '/productmonitor.html'))
                Mail::Send((int)Configuration::get('PS_LANG_DEFAULT'), 'productmonitor', Mail::l('Product out of stock', (int)Configuration::get('PS_LANG_DEFAULT')), $mailParams, explode(self::__MA_MAIL_DELIMITOR__, $this->_merchant_mails), NULL, strval(Configuration::get('PS_SHOP_EMAIL')), strval(Configuration::get('PS_SHOP_NAME')), NULL, NULL, dirname(__FILE__) . '/mails/');
        }
    }
}
